...
type AsepriteDecoderFactory = () => Promise<AsepriteDecoderModule>;
...
const loadDecoderModule = async () => {
  if (!decoderModulePromise) {
    const modulePath = `${import.meta.env.BASE_URL}wasm/aseprite-decoder.mjs`;
    decoderModulePromise = import(/* @vite-ignore */ modulePath)
      .then((module) =>
        (module.default as AsepriteDecoderFactory)(),
      )
      .catch((error: unknown) => {
